# Personal AI Study OS — Production Implementation Specification v1.2.1
...
3. **Personal AI Study OS — Production Implementation Specification v1.2.1** (This document — authoritative for database schemas, algorithms, and engineering implementation).
...
│   └── e2e/                        # Vitest E2E workflows matching 47 acceptance scenarios
...
3. **`PROCESSING`:** The Cloudflare Queue consumer has pulled the message and is actively executing provider calls. 
   - **Processing Lease:** Transition to this state MUST be an atomic conditional update that records the lease start time:
     ```sql
     UPDATE sync_jobs 
     SET status = 'PROCESSING', processing_started_at = CURRENT_TIMESTAMP
     WHERE job_id = ? AND status = 'DISPATCHED'
     ```
   - **Concurrency Safety:** The atomic condition ensures exactly one consumer claims the job.
   - **Stale PROCESSING Recovery:** If a job remains `PROCESSING` beyond a deterministic lease duration (e.g., consumer crash):
     - The system MUST NOT blindly re-run the mutation.
     - The system triggers provider reconciliation to determine whether the mutation already succeeded.
     - If already applied → mark `COMPLETED`.
     - If not applied → safe retry.
     - If uncertain → remain retryable/reconcile.
...
Explicit Release Creation:
  Create Git Release Tag (e.g. 'v1.2.1.0') via manual GitHub Release workflow
      │
      ▼
...
## 18. Authoritative Acceptance Test Suites (47 Suites)

The test suite must implement and pass all **47 explicit acceptance tests**:
...
### Stale PROCESSING & Lease Semantics Suite
9. **Consumer crashes before provider call:** Simulating consumer failure before API invocation leaves job in `PROCESSING`, which is safely retried after lease expiry.
10. **Consumer crashes during provider call:** Simulating consumer failure mid-flight triggers stale recovery and uses reconciliation to verify provider state.
11. **Consumer crashes after provider mutation but before D1 completion:** Job remains in `PROCESSING` despite external success; reconciliation correctly resolves it to `COMPLETED` without duplicate side-effects.
12. **Stale `PROCESSING` job is recovered:** A job exceeding the processing lease timeout is detected and transitioned to recovery.
13. **Provider mutation already succeeded -> reconciliation completes job without duplicate mutation:** The recovery process confirms provider presence and safely marks job `COMPLETED`.
14. **Provider mutation did not succeed -> safe retry:** The recovery process confirms provider absence and retries the mutation.
15. **Two consumers race for same job:** Concurrent consumer invocations attempt to claim the job; only one consumer succeeds.
16. **Only one consumer successfully claims processing lease:** The conditional `UPDATE sync_jobs SET status = 'PROCESSING'` ensures exact-once ownership.
17. **Queue redelivery uses `idempotency_key`:** Message redelivery checks canonical mutation identity, bypassing job execution if already processed.
18. **Stale processing job eventually reaches `COMPLETED`, `FAILED`, or `DEAD_LETTER`, never permanently stuck:** Integration test validates the lifecycle progression ensures eventual terminal state.

### Google Calendar Suite
19. **Deterministic Event ID Create Idempotency:** Submitting calendar create generates deterministic `base32hex` ID; simulated lost-ack retry catches HTTP 409 Conflict, fetches event, and completes safely.
20. **Successful ETag Update:** Updating a calendar session passes the correct `If-Match` ETag and succeeds with HTTP 200.
21. **HTTP 412 Conflict & Reconcile:** Concurrently modifying a calendar event triggers HTTP 412 Precondition Failed; adapter catches 412, refetches latest event, reconciles time blocks, and successfully updates.

### Google Tasks Suite
22. **Lost acknowledgement under 5 minutes:** Simulating lost ack under 5 minutes correctly retrieves task using metadata token without duplicating.
23. **Lost acknowledgement after 5 minutes:** Simulating lost ack after an extended period utilizes expanding search window to retrieve task without duplicating.
24. **Matching task found on page 2+:** Ensures the reconciliation loop follows `nextPageToken` and successfully finds the deterministic token on subsequent pages.
25. **No matching task after complete pagination:** Verifies that if pagination completes without finding the token, a new task is safely created.
26. **Temporary `tasks.list` failure:** A failure during the reconciliation query aborts the process and retains the uncertain retryable state instead of blind duplication.
27. **Rate-limit response during reconciliation:** A 429 response during `tasks.list` defers the message for future retry without duplication.
28. **Duplicate deterministic token prevention:** System correctly rejects creating multiple tasks with identical `[study-os:...:idempotency_key:...]` tokens.
29. **Ambiguous provider state must not trigger blind duplicate creation:** If the reconciliation state is uncertain (e.g. timeout), the adapter skips `tasks.insert` and yields.
30. **Provider State Reconciliation on Update:** Adapter fetches remote task before calling patch; if remote status is already completed, skips patch and marks completed.
31. **Date-Only Boundary Enforcement:** Verifies that time portions in study tasks are stripped, persisting strictly RFC 3339 date strings.

### Notion Suite
32. **Deterministic Property Create Reconciliation:** Database query for `OS_Entity_ID` detects existing page on retry, linking existing ID without creating duplicate page.
33. **HMAC Webhook Signature Verification:** Incoming webhook with invalid HMAC signature is rejected with HTTP 401 Unauthorized.
34. **Webhook Deduplication:** Duplicate Notion webhook deliveries are deduplicated against `idempotency_records` using `webhook_notion_<event_id>`.
35. **Latest-State Object Refetch:** Webhook processing fetches complete latest page state via `GET /v1/pages/{id}` before reconciling changes.
36. **Rate-Limit 429 & Backoff:** When Notion returns HTTP 429, consumer captures error and defers message retry following exponential backoff ($5s, 10s, 20s \dots$).

### Remote MCP Suite (MCP 2026-07-28)
37. **Streamable HTTP Transport Connection:** Client connects to `POST /mcp` via Streamable HTTP, transmitting JSON-RPC 2.0 requests and receiving streaming responses.
38. **Legacy SSE Fallback Compatibility:** Legacy clients connecting via `GET /mcp/sse` receive backward-compatible SSE streams.
39. **Raw SQL Execution Rejection:** MCP tool requests attempting raw SQL or arbitrary table drops are strictly rejected.

### Authentication Suite
40. **Expired Token Rejection:** Requests with tokens past their `exp` timestamp return `401 Unauthorized` (`INVALID_TOKEN`).
41. **Invalid Signature Rejection:** Tokens signed with untrusted keys return `401 Unauthorized`.
42. **Mandatory Audience Validation:** Valid JWTs with mismatched `aud` return `403 Forbidden` (`AUDIENCE_MISMATCH`).
43. **Insufficient Scope Rejection:** Read-only tokens attempting `POST /v1/events` return `403 Forbidden` (`INSUFFICIENT_SCOPE`).

### State & Projection Suite
44. **Canonical Event Immutability:** Ingesting an event persists immutable rows in `canonical_events`.
45. **Projection Mathematical Invariants:** `questions_attempted` events maintain $0 \le questions\_correct \le questions\_attempted$ and calculate $accuracy \in [0.0, 1.0]$.
46. **Projection Rebuild from Canonical Events:** Truncating `study_progress` and replaying canonical events restores 100% state accuracy.

### Migration & Release Gate Suite
47. **CI/CD Staging Gate & Production Promotion:** Verifies that production deployment is blocked on merge to `main` until staging verification succeeds and an explicit release tag is pushed.
...
Phase 10: Notion Adapter (HMAC Webhooks, Latest-State Refetch, 3 req/sec Rate Limiter)
  ↓
Phase 11: Observability, Acceptance Testing & CI/CD (47 Tests Suite, Staging Release Gate)
```
