# Personal AI Study OS — Production Implementation Specification v1.2.2
...
| **v1.2.2** | September 2026 | **FINAL BUILD GATE HARDENED** | **Final Build Authorization Hardening (Authoritative Baseline):**<br>• **Fully Deterministic Google Tasks Lost-Ack Recovery (Correction A):** Eliminated all obsolete fixed 5-minute search window rules across the entire specification. Codified deterministic 4-tier expanding reconciliation window (`job.created_at - 5m` $\to$ `30m` $\to$ `2h` $\to$ `24h`) with search origin anchored strictly to `sync_jobs.created_at` (never floating `now()`); established an absolute maximum search horizon of 24 hours matching `idempotency_records.expires_at` TTL; mandated complete pagination loop over `nextPageToken` with `maxResults = 100` and a 5-page (500 tasks) safety ceiling; standardized deterministic token grammar `[study-os:entity_id:<entity_id>:idempotency_key:<idempotency_key>]` with formal regex; codified fail-safe ambiguous provider state handling (HTTP 429, 5xx, timeouts, pagination interruptions yield with exponential backoff; blind create strictly prohibited); established multi-condition Safe-Create Gate; codified atomic existing-task adoption into `task_links`, `idempotency_records`, and `sync_jobs`.<br>• **Deterministic PROCESSING Lease & Atomic Stale Recovery (Correction B):** Codified exact numerical lease duration (`PROCESSING_LEASE_SECONDS = 120`); established single authoritative stale predicate (`status = 'PROCESSING' AND processing_started_at <= datetime('now', '-120 seconds')`); added dedicated D1 index `idx_sync_jobs_stale_processing(status, processing_started_at)`; codified atomic lease acquisition on consumer pull with `attempt_count` increment; codified atomic Compare-And-Set (CAS) recovery claim query preventing concurrent recovery worker races; mandated pre-retry provider reconciliation workflow across Google Calendar, Google Tasks, and Notion; codified comprehensive 7 Crash Scenarios Matrix; codified explicit non-renewable fixed lease policy for serverless Workers; codified bounded retry terminal-state safety invariant (`attempt_count < 5` else `DEAD_LETTER`).<br>• **Queue Consumer Execution Logic Hardening:** Updated Section 16.2 to mandate atomic lease acquisition step before provider adapter invocation.<br>• **Acceptance Test Consistency:** Reconciled automated test matrix to 50 authoritative test scenarios, guaranteeing 1:1 mapping with the deterministic contracts for Google Tasks lost-ack (Tests 22–34) and PROCESSING lease / crash recovery (Tests 9–21).<br>• **Contract Authority Confirmation:** Confirmed that **Technical Contracts v1.1 remains authoritative and unchanged.** |
...
3. **Personal AI Study OS — Production Implementation Specification v1.2.2** (This document — authoritative for database schemas, algorithms, and engineering implementation).
...
[ Periodic Cron Trigger (Every 60 Seconds) ]
  ...
  ASC LIMIT 50;           processing_started_at = now()
         ...
         ▼                AND processing_started_at = :orig_ts;
  SYNC_QUEUE.send(...)           │
         │                ┌──────┴──────┐
         ▼                ▼ (changes=1) ▼ (changes=0)
  UPDATE sync_jobs        [ Execute     [ Yield: Another
  SET status=             Provider      Worker Claimed ]
...
- If `changes === 0`: Another consumer claimed the job or it was completed concurrently; the consumer calls `msg.ack()` and terminates immediately without calling any external API.

#### B4. Atomic Compare-And-Set (CAS) Recovery Ownership
When the Cron Dispatcher (Sweep Query 2) discovers a stale job, it MUST NOT blindly retry the mutation. It claims exclusive recovery ownership via an atomic compare-and-set query:
...
- If `changes === 0`: The worker yields immediately. Application-level "check then update" logic is strictly forbidden.
...
```text
STALE PROCESSING (Claimed via CAS)
               │
               ▼
...
(Calendar GET deterministic ID / Tasks updatedMin list / Notion OS_Entity_ID query)
  ...
  [APPLIED] [ABSENT] [UNCERTAIN]
      │        │        │
      │        │        └─── Mark FAILED, next_attempt_at = now + backoff, yield
      │        │
      │        └─── Check attempt_count:
      │               ├── If attempt_count >= 5: Mark DEAD_LETTER, route to DLQ
      │               └── If attempt_count < 5: Mark DISPATCHED, re-enqueue SYNC_QUEUE
      │
...
#### B7. Terminal-State Safety Invariant
A stale job must never cycle indefinitely between `PROCESSING` and recovery:
1. Every claim (consumer lease claim or recovery CAS claim) increments `attempt_count = attempt_count + 1`.
2. When `attempt_count >= 5`, the recovery runner routes the job to `personal-sync-dlq` and executes:
   ```sql
   UPDATE sync_jobs
   SET status = 'DEAD_LETTER',
       last_error = 'EXCEEDED_MAX_RECOVERY_ATTEMPTS',
       completed_at = CURRENT_TIMESTAMP,
       updated_at = CURRENT_TIMESTAMP
   WHERE job_id = ?;
   ```
3. A job is guaranteed to reach `COMPLETED`, `FAILED`, or `DEAD_LETTER`. Permanent stranded execution in `PROCESSING` is mathematically impossible.
...
| :--- | :--- | :--- |
| **`PENDING`** | Durably written to D1; awaits in-line dispatch or cron sweep pickup. | `DISPATCHED`, `PROCESSING` (fast path) |
| **`DISPATCHED`** | Accepted by `SYNC_QUEUE`; awaits consumer pull. | `PROCESSING` |
| **`PROCESSING`** | Claimed by an active consumer or recovery worker under a 120-second lease. | `COMPLETED`, `FAILED`, `DEAD_LETTER`, `DISPATCHED` (via recovery) |
| **`COMPLETED`** | External mutation verified and acknowledged; `idempotency_records` updated. | *Terminal state* |
| **`FAILED`** | Transient error encountered (429, 5xx); backoff scheduled in `next_attempt_at`. | `DISPATCHED` (via sweep) |
| **`DEAD_LETTER`**| Exceeded 5 attempts; routed to `personal-sync-dlq` for human operator inspection. | *Terminal state* |
...
| :---: | :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | **Worker crash after D1 batch commit, before Queue enqueue** | `status = 'PENDING'`, `dispatched_at IS NULL` | Nothing invoked | Sweep Query 1 discovers `PENDING` with `created_at <= now - 30s` | Dispatcher sends message to `SYNC_QUEUE`, updates `status = 'DISPATCHED'`, `dispatched_at = now()` | Enqueued for processing; reaches `COMPLETED` or `DEAD_LETTER` |
...
| **3** | **Consumer crash immediately after claiming lease, before provider API call** | `status = 'PROCESSING'`, `processing_started_at = T0` | Nothing invoked | Sweep Query 2 discovers `status = 'PROCESSING'` and `now >= T0 + 120s` | CAS recovery claim; reconciliation confirms remote entity absent; checks attempts; retries or routes to DLQ | Guaranteed terminal state via `attempt_count` bound ($\le 5$) |
| **4** | **Consumer crash while external provider API call is in-flight** | `status = 'PROCESSING'`, `processing_started_at = T0` | Unknown (may have executed, timed out, or dropped) | Sweep Query 2 discovers `status = 'PROCESSING'` and `now >= T0 + 120s` | CAS recovery claim; executes provider reconciliation: if found $\to$ `COMPLETED`; if absent $\to$ retry or DLQ | No duplicate external mutation; reaches `COMPLETED` or `DEAD_LETTER` |
| **5** | **Consumer crash after provider mutation succeeds, before D1 `COMPLETED` update** | `status = 'PROCESSING'`, `idempotency_records = 'PENDING'` | Successfully executed in provider | Sweep Query 2 discovers `status = 'PROCESSING'` and `now >= T0 + 120s` | CAS recovery claim; reconciliation verifies remote presence; marks `sync_jobs` and `idempotency_records` as `COMPLETED` | Successfully resolved to `COMPLETED` without repeating provider call |
...
| **7** | **Crash of recovery worker during stale recovery / reconciliation** | `status = 'PROCESSING'`, `processing_started_at = T1` (from CAS claim) | Remote state remains static | Subsequent Sweep Query 2 discovers `status = 'PROCESSING'` and `now >= T1 + 120s` | Subsequent CAS recovery claim; re-executes reconciliation cleanly | Handled recursively; bounded by `attempt_count <= 5` to prevent infinite recovery |
...
### 10.3 Google Tasks Integration Contract (Authoritative v1.2.2 Hardened)
...
The `updatedMin` parameter expands deterministically based on `sync_jobs.attempt_count`:

```text
Tier 1 (Attempt 0–1): updatedMin = ISO8601(sync_jobs.created_at - 5 minutes)
Tier 2 (Attempt 2):   updatedMin = ISO8601(sync_jobs.created_at - 30 minutes)
Tier 3 (Attempt 3):   updatedMin = ISO8601(sync_jobs.created_at - 2 hours)
Tier 4 (Attempt 4–5): updatedMin = ISO8601(sync_jobs.created_at - 24 hours)
```
...
- **HTTP 429 (Rate Limit):** The adapter aborts immediately, increments `sync_jobs.attempt_count`, sets `next_attempt_at` with exponential backoff ($5s, 10s, 20s \dots$), and calls `msg.retry({ delaySeconds })`. `tasks.insert` is **NEVER** called.
- **HTTP 5xx / Network Timeout:** The adapter captures the error, records `sync_jobs.last_error`, and throws a retryable exception. `tasks.insert` is **NEVER** called.
- **Pagination Ceiling Reached (500 tasks):** The adapter yields with a retryable error, preventing a catastrophic duplicate creation storm.
...
### 16.2 Consumer Execution Logic (v1.2.2 Hardened)
The worker's `queue` handler processes batches with deduplication, atomic lease claims, and concurrency control:
...
4. **Atomic Processing Lease Claim:** Executes atomic conditional update to claim execution lease:
   ```sql
   UPDATE sync_jobs
   ...
   ```
   If `changes === 0`: Another consumer claimed or completed the job; call `msg.ack()` and terminate execution immediately.
5. **Provider Adapter Dispatch:** Calls target provider adapter (`GoogleTasksAdapter`, `GoogleCalendarAdapter`, `NotionAdapter`).
6. **Success Branch:** Updates `sync_jobs.status = 'COMPLETED'`, `idempotency_records.status = 'COMPLETED'`, and calls `msg.ack()`.
7. **Transient Failure Branch (HTTP 429/503/Timeout):** Increments `attempt_count`, calculates exponential backoff delay, updates `sync_jobs.status = 'FAILED'`, and calls `msg.retry({ delaySeconds })`.
8. **Terminal Failure Branch (Attempt 5):** Message routed to `personal-sync-dlq`; updates `sync_jobs.status = 'DEAD_LETTER'`.
...
Explicit Release Creation:
  Create Git Release Tag (e.g. 'v1.2.2.0') via manual GitHub Release workflow
      │
      ▼
...
### Stale PROCESSING & Lease Semantics Suite (13 Tests)
...
11. **Atomic stale recovery CAS claim:** Recovery runner successfully claims stale job using compare-and-set query on `processing_started_at`.
12. **Crash before provider call:** Simulating consumer failure before API invocation leaves job in `PROCESSING`, which is safely recovered and retried after 120-second lease expiry.
13. **Crash during provider call:** Simulating consumer failure mid-flight triggers stale recovery and uses reconciliation to verify provider state.
14. **Crash after provider mutation but before D1 completion:** Job remains in `PROCESSING` despite external success; reconciliation correctly resolves it to `COMPLETED` without duplicate side-effects.
15. **Provider timeout after remote mutation:** Adapter times out during response read; stale recovery discovers remote entity presence and marks `COMPLETED`.
16. **Provider reconciliation success:** Recovery process confirms provider presence and safely transitions `sync_jobs` and `idempotency_records` to `COMPLETED`.
17. **Provider reconciliation absence:** Recovery process confirms provider absence and transitions job to safe retry.
18. **Provider reconciliation uncertainty:** Recovery process receives 429 or timeout during reconciliation; job transitions to `FAILED` with backoff and remains retryable without blind mutation.
19. **Concurrent stale recovery race:** Two recovery runners simultaneously attempt to claim the same stale job; conditional CAS ensures exactly one succeeds.
20. **Duplicate queue redelivery:** Queue redelivery checks canonical mutation identity, bypassing job execution if already processed.
21. **Eventual terminal state:** Integration test validates that crashed jobs increment `attempt_count` on each claim and reach `DEAD_LETTER` after 5 attempts, never permanently stuck.
...
### Google Tasks Suite (13 Tests)
25. **Initial reconciliation window search:** Simulating lost ack under attempt 0–1 uses `job.created_at - 5 minutes` and retrieves task without duplicating.
26. **First expansion window search:** Simulating lost ack at attempt 2 uses expanded window `job.created_at - 30 minutes` and retrieves task.
27. **Maximum reconciliation horizon:** Tasks adapter queries up to `job.created_at - 24 hours` on attempt 4–5; historical search beyond 24 hours is rejected.
...
32. **Rate-limit 429 during reconciliation:** HTTP 429 during `tasks.list` defers message with exponential backoff without calling `tasks.insert`.
33. **Provider 5xx error during reconciliation:** HTTP 500/503 during `tasks.list` prevents task creation and defers job.
...
42. **Rate-Limit 429 & Backoff:** When Notion returns HTTP 429, consumer captures error and defers message retry following exponential backoff ($5s, 10s, 20s \dots$).
...
- **ADR-016: Deterministic 120-Second Outbox Processing Lease & Atomic CAS Stale Recovery:** Mandated fixed non-renewable 120-second lease duration, atomic compare-and-set stale recovery claim, pre-retry provider reconciliation across all adapters, and guaranteed terminal-state progression.
...
Version: 1.2.2
Gate 3: FINAL BUILD GATE HARDENED
Architecture: GREEN
